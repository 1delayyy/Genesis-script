...
menu.action(MenuGameRunMacrosReg, "Register as CEO", {"gsmacrosceo"}, "Runs a Macro that will Register you as a CEO.", function(on_click)
...
menu.action(MenuGameRunMacrosReg, "Register as MC", {"gsmacrosmc"}, "Runs a Macro that will Register you as an MC President.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())   
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
    ...
    else util.toast("-Genesis-\n\nYou are Already a MC President! If you are a CEO or VIP, then use 'Swap Registration' Macro.") end
end)

menu.action(MenuGameRunMacrosReg, "Swap Registration", {"gsmacrossr"}, "Runs a Macro that will Change your Registration from CEO to MC or Vice Versa, based on which one you're on.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == 0 then
...
menu.action(MenuGameRunMacrosSur, "Drop BST", {"gsmacrosbst"}, "Runs a Macro that will Drop BST.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == 0 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosSur, "Drop Armour", {"gsmacrosarmour"}, "Runs a Macro that will Drop Armour.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == 0 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosSur, "Old EWO", {"gsmacrosewo"}, "Runs a Macro that will use the Old GTAO Tryhard EWO Method.", function(on_click)
    util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosAbi, "Ghost Organization", {"gsmacrosghost"}, "Runs a Macro that will Quickly put you in Ghost Organization.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == 0 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosAbi, "Bribe Authorities", {"gsmacrosbribe"}, "Runs a Macro that will Bribe Authorities.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == 0 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosVeh, "Spawn Buzzard", {"gsmacrosbuzzard"}, "Runs a Macro that will Call a Buzzard.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == 0 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosVeh, "Spawn Sparrow", {"gsmacrossparrow"}, "Runs a Macro that will Call a Sparrow.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosPhone, "Call Lamar", {"gsmacrocalllamar"}, "Calls Lamar on your Phone.", function(on_click)
    util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosPhone, "Call Lester", {"gsmacrocalllester"}, "Calls Lester on your Phone.", function(on_click)
    util.yield(macroRunDelay)
    if menu.is_open() == true then
        util.toast("Macro is Ready. Close Stand and it will Immediately Run.")
        while menu.is_open ~= false do
...
menu.action(MenuGameRunMacrosPhone, "Call Mechanic", {"gsmacrocallmechanic"}, "Calls the Mechanic on your Phone.", function(on_click)
    util.yield(macroRunDelay)
    if menu.is_open() == true then
        util.toast("Macro is Ready. Close Stand and it will Immediately Run.")
        while menu.is_open ~= false do
...
menu.action(MenuGameRunMacrosPhone, "Call Merrywether", {"gsmacrocallmerrywether"}, "Calls Merrywether on your Phone.", function(on_click)
    util.yield(macroRunDelay)
    if menu.is_open() == true then
        util.toast("Macro is Ready. Close Stand and it will Immediately Run.")
        while menu.is_open ~= false do
...
menu.action(MenuGameRunMacrosPhone, "Call MMI", {"gsmacrocallmmi"}, "Calls Mors Mutual Insurance on your Phone.", function(on_click)
    util.yield(macroRunDelay)
    if menu.is_open() == true then
        util.toast("Macro is Ready. Close Stand and it will Immediately Run.")
        while menu.is_open ~= false do
...
menu.action(MenuGameRunMacrosPhone, "Call Pegasus", {"gsmacrocallpegasus"}, "Calls Pegasus on your Phone.", function(on_click)
    util.yield(macroRunDelay)
    if menu.is_open() == true then
        util.toast("Macro is Ready. Close Stand and it will Immediately Run.")
        while menu.is_open ~= false do
...
menu.action(MenuGameRunMacrosServ, "Request MOC", {"gsmacrossmoc"}, "Runs a Macro that will Call your Mobile Operations Centre.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosServ, "Request Avenger", {"gsmacrosavenger"}, "Rans a Macro that will Call your Avenger.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosServ, "Request Terrobyte", {"gsmacrosterrobyte"}, "Rans a Macro that will Call your Terrobyte.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosServ, "Request Kosatka", {"gsmacroskosatka"}, "Runs a Macro that will Call your Kosatka.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
...
menu.action(MenuGameRunMacrosMisc, "Activate Thermal Helmet", {"gsmacrosthermal"}, "Runs a Macro that will Activate the Thermal Helmet if it is Down.", function(on_click)
    local typeOfCEO = getPlayerRegType(players.user())
    if typeOfCEO == -1 then
        util.yield(macroRunDelay)
...
menu.action(MenuMisc, "Patch Notes", {"gspatchnotes"}, "New feature's i've added.", function(on_click)
    util.toast("-Genesis-\n\n-Fixed-\n"..PatchNoteFixed.."\n\n-Added-\n"..PatchNoteAdded)
end)
...
function PlayerAddRoot(csPID)
    ...
    menu.hyperlink(MenuPlayerFun, "Text / Label List", "https://github.com/1delayyy/Genesis")
